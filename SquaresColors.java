...
public class SquaresColors extends Frame
...
class newSquaresColors extends Canvas
{  int maxX, maxY, minMaxXY, xCenter, yCenter;
   ...
   {  
		...
		for (int k = 0; k < 8; k++){
			for(int j = 0; j < 8; j++){
				xA = (xCenter + sideHalf)/4 + (160*k); yA = (yCenter - 0.5F * h)/2 + (160*j);
				xB = (xCenter - sideHalf)/4 + (160*k); yB = yA/2 - (160*j);
				xC = (xCenter - sideHalf)/4 + (160*k); yC = (yCenter + 0.5F * h)/2 + (160*j);
				xD = (xCenter + sideHalf)/4 + (160*k); yD = yC/2 - (160*j);
